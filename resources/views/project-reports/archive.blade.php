...
@section('page_title', __('project_reports.archive_title'))
@section('page_subtitle', __('project_reports.archive_subtitle'))
...
<div class="dashboard-stack">
    <section class="dashboard-panel">
        <div class="panel-head" style="display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap;">
            <div>
                <h2 class="panel-title">{{ __('project_reports.archive_title') }}</h2>
                <p class="panel-subtitle">{{ __('project_reports.archive_subtitle') }}</p>
            </div>
        ...
        <form method="GET" action="{{ route('project-reports.archive') }}" style="margin-top:12px;">
            <div class="form-grid" style="align-items:end;">
                <div class="form-group">
                    <label>{{ __('project_reports.archive_year') }}</label>
                    <select name="year">
                ...
                <div class="form-group">
                    ...
                    <a href="{{ route('project-reports.archive', ['year' => $year]) }}" class="btn btn-secondary">{{ __('common.reset') }}</a>
                </div>
            </div>
        </form>
        ...
        <div class="stats-grid" style="margin-top:18px;">
            @forelse($projects as $project)
                <div class="stat-card" style="display:flex; flex-direction:column; gap:10px; min-height:160px;">
                    ...
                    <div style="font-size:12px; color:#111827; line-height:1.7;">
                        ...
                        <div><strong>{{ __('project_reports.card_status') }}:</strong> {{ __('project_reports.status_completed') }}</div>
                        <div><strong>{{ __('project_reports.card_completed_at') }}:</strong>
            ...
            @empty
                <div class="stat-card" style="grid-column:1 / -1;">
                    <div class="empty-row">{{ __('project_reports.archive_empty') }}</div>
                </div>
            @endforelse
        </div>
    </section>
